;;; otrenav-zerodark-theme.el --- A dark, medium contrast theme for Emacs -*- lexical-binding: t -*-
...
;; URL: https://github.com/NicolasPetton/otrenav-zerodark-theme
;; Version: 3.6
;; Package: otrenav-zerodark-theme
...
;;; License:
;;
;; Otrenav-Zerodark (https://github.com/otrenav-zerodark-theme) is licensed under a
...
;; `otrenav-zerodark-setup-modeline-format'.
;;
...
(deftheme otrenav-zerodark
  "A dark medium contrast theme")

(defgroup otrenav-zerodark
  nil
  "A dark theme inspired from One Dark and Niflheim.")

(defcustom otrenav-zerodark-use-paddings-in-mode-line t
  "When non-nil, use top and bottom paddings in the mode-line."
  :type 'boolean
  :group 'otrenav-zerodark)

(defface otrenav-zerodark-ro-face
  '((t :foreground "#0088CC" :weight bold))
  "Face for read-only buffer in the mode-line.")

(defface otrenav-zerodark-modified-face
  '((t :foreground "#ff6c6b" :height 0.9))
  "Face for modified buffers in the mode-line.")

(defface otrenav-zerodark-not-modified-face
  '((t :foreground "#98be65" :height 0.9))
  "Face for not modified buffers in the mode-line.")

(defface otrenav-zerodark-buffer-position-face
  '((t :height 0.9))
  "Face for line/column numbers in the mode-line.")

(defface otrenav-zerodark-vc-face
  '((t :foreground "#61afef"))
  "Face for vc status in the mode-line.")

(defface otrenav-zerodark-ok-face
  '((t :foreground "#61afef"))
  "Face for ok status in the mode-line.")

(defface otrenav-zerodark-warning-face
  '((t :foreground "#da8548"))
  "Face for warning status in the mode-line.")

(defface otrenav-zerodark-error-face
  '((t :foreground "#ff6c6b"))
  "Face for error status in the mode-line.")

(defvar otrenav-zerodark-modeline-position '(:eval (propertize ":%l:%c %p " 'face (if (otrenav-zerodark--active-window-p))
                                                                              'otrenav-zerodark-buffer-position-face
                                                                            'mode-line-inactive))
  "Mode line construct for displaying the position in the buffer.")

(defvar otrenav-zerodark-modeline-buffer-identification '(:eval (propertize "%b" 'face 'bold))
  "Mode line construct for displaying the position in the buffer.")

(defvar otrenav-zerodark-modeline-modified '(:eval (if (buffer-modified-p (current-buffer)))
                                               (all-the-icons-faicon "floppy-o"
                                                                     :height 0.9
                                                                     :v-adjust 0
                                                                     :face (if (otrenav-zerodark--active-window-p)
                                                                               'otrenav-zerodark-modified-face
                                                                             'mode-line-inactive))
                                             (all-the-icons-faicon "check"
                                                                   :height 0.9
                                                                   :v-adjust 0
                                                                   :face (if (otrenav-zerodark--active-window-p)
                                                                             'otrenav-zerodark-not-modified-face
                                                                           'mode-line-inactive))))

(defvar otrenav-zerodark-modeline-ro '(:eval (if buffer-read-only)
                                         (if (otrenav-zerodark--active-window-p)
                                             (progn
                                               (propertize "RO " 'face 'otrenav-zerodark-ro-face))
                                           (propertize "RO " 'face 'bold))
                                       ""))

(defvar otrenav-zerodark-buffer-coding '(:eval (unless (eq buffer-file-coding-system (default-value 'buffer-file-coding-system)))
                                         mode-line-mule-info))

(defvar otrenav-zerodark-modeline-vc '(vc-mode ("   ")
                                        (:eval (all-the-icons-faicon "code-fork"
                                                                     :height 0.9
                                                                     :v-adjust 0
                                                                     :face (when (otrenav-zerodark--active-window-p)
                                                                             (otrenav-zerodark-git-face))))
                                        (:eval (propertize (truncate-string-to-width vc-mode 25 nil nil "...")
                                                           'face (when (otrenav-zerodark--active-window-p)
                                                                   (otrenav-zerodark-git-face))))))

(defun otrenav-zerodark-modeline-flycheck-status ()
                   ...
                   (`finished (if flycheck-current-errors
                                  (let ((count (let-alist (flycheck-count-errors flycheck-current-errors)
                                    ...
                                    (propertize (format "✖ %s Issue%s" count (if (eq 1 count) "" "s"))
                                                'face (otrenav-zerodark-face-when-active 'otrenav-zerodark-error-face)))
                                (propertize "✔ No Issues"
                                            'face (otrenav-zerodark-face-when-active 'otrenav-zerodark-ok-face))))
                   (`running     (propertize "⟲ Running"
                                             'face (otrenav-zerodark-face-when-active 'otrenav-zerodark-warning-face)))
...
(defvar otrenav-zerodark--git-face-cached (cached-for 1 (otrenav-zerodark--git-face-intern)))

(defun otrenav-zerodark--git-face-intern ()
  ...
  (if (magit-git-success "diff" "--quiet")
      ...
      (if (zerop (length (magit-git-string
          ...
          'otrenav-zerodark-ok-face
        ;; nothing to commit, but some commits must be pushed
        'otrenav-zerodark-warning-face)
    'otrenav-zerodark-error-face))

(defun otrenav-zerodark-git-face ()
  "Return the face to use based on the current repository status.
The result is cached for one second to avoid hiccups."
  (funcall otrenav-zerodark--git-face-cached))
  ...
  (custom-theme-set-faces
   ...
   ;; Mode line faces
      `(mode-line ((,class (:background ,background-blue :height 0.9 :foreground ,blue
                                     :box ,(when otrenav-zerodark-use-paddings-in-mode-line
   ...
   ;; neotree
   `(neo-file-link-face ((t (:foreground ,comment))))
   `(neo-dir-link-face ((t (:foreground ,blue))))

   ;; tabbar
   `(tabbar-button ((t (:foreground ,default
                                    :background ,background))))
   `(tabbar-selected ((t (:foreground ,default
                                      :background ,background-dark
                                      :box (:line-width -1 :style pressed-button)))))
   `(tabbar-unselected ((t (:foreground ,default
                                        :background ,background
...
(defun otrenav-zerodark-face-when-active (face)
  "Return FACE if the window is active."
  (when (otrenav-zerodark--active-window-p)
    face))

;; So the mode-line can keep track of "the current window"
(defvar otrenav-zerodark-selected-window nil
  "Selected window.")

(defun otrenav-zerodark--set-selected-window (&rest _)
  "Set the selected window."
  (let ((window (frame-selected-window)))
    (when (and (windowp window)
               (not (minibuffer-window-active-p window)))
      (setq otrenav-zerodark-selected-window window))))

(defun otrenav-zerodark--active-window-p ()
  "Return non-nil if the current window is active."
  (eq (selected-window) otrenav-zerodark-selected-window))

(add-hook 'window-configuration-change-hook #'otrenav-zerodark--set-selected-window)
(add-hook 'focus-in-hook #'otrenav-zerodark--set-selected-window)
(advice-add 'select-window :after #'otrenav-zerodark--set-selected-window)
(advice-add 'select-frame  :after #'otrenav-zerodark--set-selected-window)


;;;###autoload
(defun otrenav-zerodark-setup-modeline-format ()
  "Setup the mode-line format for otrenav-zerodark."
  (interactive)
  (require 'flycheck)
  (require 'magit)
  (let ((class '((class color) (min-colors 89)))
  ...
  (setq-default mode-line-format
                `("%e"
                  " "
                  ,otrenav-zerodark-modeline-ro " "
                  ,otrenav-zerodark-buffer-coding
                  mode-line-frame-identification " "
                  " "
                  ,otrenav-zerodark-modeline-modified
                  " "
                  ,otrenav-zerodark-modeline-buffer-identification
                  ,otrenav-zerodark-modeline-position
                  ,otrenav-zerodark-modeline-vc
                  "  "
                  (:eval (otrenav-zerodark-modeline-flycheck-status))
...
(provide-theme 'otrenav-zerodark)

;; Local Variables:
;; no-byte-compile: t
;; End:

;;; otrenav-zerodark-theme.el ends here
